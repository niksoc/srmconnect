...
class MultiListViewPage extends React.Component{
    	...
    	axios.get(this.construct_data_url(props, model)) 
	        .then(({data})=> {
                let data1 = this.state.data;
                    if(this.props.route.mainTitle==='Latest') data1[model] = data.slice(0,2); 
		    else data1[model]=data;
	...
	const features = this.props.route.features.map((feature,i)=><div key={i}><PageTitle model={feature.model} title={feature.title} style={inlineBlock} 
	...
	return ( 
		<div> 
		<PageHeader style={{borderColor:'black'}}>{this.props.route.mainTitle}</PageHeader>
		{features}
		</div>
	);
    }
